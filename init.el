...
;; Python のインポート文の順番を整理する．
;; 本パッケージを利用するには，isort をインストールする必要がある．
;;
;;     > pip install isort
(use-package py-isort
  :ensure t
  :config
  (message ":config py-isort")
  )
...
(add-hook
 ...
 '(lambda ()
    ...
    (when (eq system-type 'windows-nt)
      ...
      ;; NOTE 2026-01-17: Gemini 3 Pro の指摘に従い，以下に変更
      (set-fontset-font
       t 'japanese-jisx0208 (font-spec :family "Migu 1M" :height 120))
      (set-fontset-font
       t 'japanese-jisx0213.2004-1 (font-spec :family "Migu 1M" :height 120))
